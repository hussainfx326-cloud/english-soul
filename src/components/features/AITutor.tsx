...
import { Send, Bot, User, Sparkles, Mic, Paperclip, X } from "lucide-react";
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "assistant",
      content: "Hello Alex! I am your AI native tutor. Are there any grammar rules or sentences you want me to check today?",
    },
    {
      id: "2",
      role: "user",
      content: "I want to say that I am living here since 5 years.",
    },
    {
      id: "3",
      role: "assistant",
      content: "Great effort! However, there's a small grammar issue there. Let's fix it:",
      correction: {
        original: "I am living here since 5 years.",
        fixed: "I have been living here for 5 years.",
        explanation: "When talking about an action that started in the past and continues to the present, we use the Present Perfect Continuous tense ('have been living'). Also, when measuring duration (5 years), we use 'for', not 'since'."
      }
    }
  ]);
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, {
        ...
        correction: data.correction?.original ? data.correction : undefined
      }]);
    } catch (error) {
      ...
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: "assistant",
        content: "Sorry, I am having trouble connecting to my brain right now. Please check your API keys."
      }]);
    }
  };
  ...
  return (
    <div className="w-full h-full max-h-[800px] bg-background border border-border-glass rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
      ...
      <header className="h-20 bg-background/80 backdrop-blur-xl border-b border-border-glass px-6 flex items-center justify-between z-10 shrink-0">
        ...
        <button className="p-2 text-foreground/40 hover:text-foreground transition-colors hover:bg-foreground/5 rounded-full">
           <X className="w-6 h-6" />
        </button>
      </header>
      ...
      <main className="flex-1 overflow-y-auto p-6 space-y-6 z-10">
        {messages.map((msg) => (
          ...
          >
            ...
            <div className={`max-w-[85%] md:max-w-[70%] space-y-3 ${msg.role === "user" ? "items-end" : "items-start"}`}>
              {/* Main Message Bubble */}
              <div 
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <p className="text-color-danger-900/60 line-through decoration-color-danger-500/50">"{msg.correction.original}"</p>
                      </div>

                      <div className="space-y-1">
                        <span className="text-xs font-bold text-color-success-600 uppercase tracking-wider">Say</span>
                        <p className="text-color-primary-900 font-bold text-lg">"{msg.correction.fixed}"</p>
                      </div>
            ...
            {msg.role === "user" && (
              <div className="w-8 h-8 rounded-full bg-color-accent-100 flex items-center justify-center shrink-0 mt-1">
                <span className="font-bold text-color-accent-600 text-xs">A</span>
              </div>
            )}
          </div>
        ))}
      ...
      <footer className="p-4 bg-background border-t border-border-glass z-10 shrink-0">
        ...
        >
          ...
          <textarea 
            ...
            className="w-full bg-transparent border-none focus:outline-none resize-none min-h-[44px] max-h-[120px] py-3 text-foreground placeholder:text-foreground/40 font-sans leading-relaxed"
            rows={1}
          />
          
          {input.length === 0 ? (
            <button type="button" className="p-3 text-white bg-gradient-to-tr from-color-primary-600 to-color-accent-500 hover:from-color-primary-500 hover:to-color-accent-400 rounded-xl transition-colors shrink-0 shadow-lg shadow-color-primary-500/20 active:scale-95">
              <Mic className="w-5 h-5" />
            </button>
          ) : (
            <button type="submit" className="p-3 text-white bg-color-primary-600 hover:bg-color-primary-500 rounded-xl transition-colors shrink-0 shadow-lg shadow-color-primary-500/20 active:scale-95 animate-in zoom-in duration-200">
